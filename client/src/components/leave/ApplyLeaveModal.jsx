...
const ApplyLeaveModal = ({open, onClose, onSuccess}) => {
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm' onClick={onClose}>
        ...
        <div className='relative bg-white rounded-2x1 shadow-2x1 w-full max-w-lg animate-fade-in' onClick={(e)=>e.stopPropagation()}>
            {/* ----- Header -----*/}
            ...
            <form onSubmit={handleSubmit} className='p-6 space-y-5'>
                ...
                <div>
                   <label className='flex items-center gap-2 text-sm font-medium text-slate-700 mb-2'>
                       <CalandarDays className="w-4 h-4 text-slate-400"/>
                       Duration 
                    </label>
                ...
                <div>
                    ...
                    <textarea name="reason" required rows={3} className="resize-none" placeholder="Briefly describe why you need tgis leave..." />
                </div>

                {/* ---- buttons ----- */}
                <div className="flex gap-3 pt-2">
                    <button onClick={onClose} type='button' className="btn-secondry flex-1">
                        Cancel
                    </button>
